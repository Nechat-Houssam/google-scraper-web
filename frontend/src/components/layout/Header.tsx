  ...
  return (
    <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
      ...
      <button 
        ...
        className={`flex items-center gap-3 px-10 py-5 rounded-2xl font-black text-xl shadow-xl transition-all active:scale-95 ${
          loading ? 'bg-red-500 hover:bg-red-600 animate-pulse' : 'bg-blue-600 hover:bg-blue-700'
        } text-white`}
      >
        {loading ? <><X size={24}/> ANNULER LE SCAN</> : <><Play fill="currentColor"/> LANCER LE SCAN</>}
      </button>
    </header>
  );
};